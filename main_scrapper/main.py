...
class ScraperWorker(threading.Thread):
    ...
    def run(self):
        ...
        self.log.info(
            f"Started | poll: {self.poll_min}-{self.poll_max}s"
        )
        ...
        while not self.stop_event.is_set():
            ...
            if new_items:
                try:
                    ...
                    self.last_error = f"storage save error: {type(e).__name__}: {e}"
                    self.last_error_time = time.time()
                    self.log.error(f"Storage save error: {e}")
                    self.consecutive_errors += 1
                    self._check_blocked()
                    self._backoff_sleep()
                    continue
                ...
                self.log.info(
                    f"+{saved} new | Total: {self.total_fetched}"
                )
    ...
    def _check_blocked(self):
        ...
        if self.consecutive_errors >= max_errors or elapsed >= threshold:
            ...
            self.block_count += 1
            self.log.blocked(
                f"BLOCKED — {self.consecutive_errors} consecutive errors, "
                f"{elapsed/60:.0f}m since last success "
                f"(escalation level {self.block_count})"
            )
            if self.redis:
...
def main():
    ...
    if config["dashboard_enabled"]:
        dashboard_server = start_dashboard(storage, workers, config, start_time)
        if dashboard_server:
