...
class DashboardHandler(SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path in ("/docs", "/docs/", f"{URL_PREFIX}/docs", f"{URL_PREFIX}/docs/"):
            self._serve_swagger_ui()
            return

        if path in ("/openapi.json", f"{URL_PREFIX}/openapi.json"):
            self._send_json(build_openapi_spec(self.config))
            return

        if path.startswith("/api/") or path.startswith(f"{URL_PREFIX}/api/"):
    ...
    def _serve_swagger_ui(self):
        # Swagger UI is loaded from CDN and points to prefixed OpenAPI path.
        body = b"""<!doctype html>
<html lang=\"en\">
  <head>
    <meta charset=\"utf-8\" />
  ...
  <body>
    ...
    <script>
      window.ui = SwaggerUIBundle({
                url: '/news_scrapper/openapi.json',
        dom_id: '#swagger-ui',
        deepLinking: true,
...
def start_dashboard(storage, workers: list, config: dict, start_time: float, top_news_mgr=None):
    ...
    DashboardHandler.api_router = HealthApiRouter(
        ApiControllers(storage, workers, config, start_time, PROJECT_ROOT, top_news_mgr=top_news_mgr)
    )
